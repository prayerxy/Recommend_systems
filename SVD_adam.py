...
# latent factor model
class mySVD:
    def __init__(self,items,users,item_user_matrix,lamda1=0.03,lamda2=0.03,lamda3=0.03,lamda4=0.03,factors=30,lr=0.006):
        ...
        self.optim_configs={'bx':{},'bi':{},'Q':{},'P':{}} # 优化器的配置参数
            ...
            for epoch in range(self.n_epochs):
                ...
                for item, users in tqdm(self.train_data.items(), desc=f"Progress [{epoch+1}/{self.n_epochs}]", total=len(self.train_data)):
                    for user, rating in users:
                        ...
                        #计算梯度
                        grad_bx[user]+=-2*error+self.lamda1*self.bx[user]
                        grad_bi[item]+=-2*error+self.lamda2*self.bi[item]
                        grad_Q[item]+=-2*error*self.P[user]+self.lamda3*self.Q[item]
                        grad_P[user]+=-2*error*self.Q[item]+self.lamda4*self.P[user]
                #更新参数
                self.bx,self.optim_configs['bx']=adam(self.bx,grad_bx,self.optim_configs['bx'])
                self.bi,self.optim_configs['bi']=adam(self.bi,grad_bi,self.optim_configs['bi'])
                self.Q,self.optim_configs['Q']=adam(self.Q,grad_Q,self.optim_configs['Q'])
                self.P,self.optim_configs['P']=adam(self.P,grad_P,self.optim_configs['P'])
